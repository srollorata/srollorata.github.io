...
function updateProjectInfo(project) {
  ...
  if (project.name) {
    infoHTML += `<li><strong>Name</strong>: ${escapeHtml(project.name)}</li>`;
  }
  
  infoHTML += `
    <li><strong>Category</strong>: ${escapeHtml(project.category)}</li>
    ${project.client ? `<li><strong>Client</strong>: ${escapeHtml(project.client)}</li>` : ""}
    <li><strong>Project date</strong>: ${escapeHtml(project.projectDate)}</li>
    <li><strong>Tools Used</strong>: ${escapeHtml(project.toolsUsed)}</li>
    ${project.relatedEvent ? `<li><strong>Related Event</strong>: ${escapeHtml(project.relatedEvent)}</li>` : ""}
    ${project.description ? `<li><strong>Description</strong>: ${escapeHtml(project.description)}</li>` : ""}
    ${project.projectUrl ? `<li><strong>Project URL</strong>: <a href="${escapeHtml(project.projectUrl)}" target="_blank">${escapeHtml(project.projectUrl)}</a></li>` : ""}
    ${project.projectGuide ? `<li><strong>Project Guide</strong>: <a href="${escapeHtml(project.projectGuide)}" target="_blank">${escapeHtml(project.projectGuide)}</a></li>` : ""}
    ${project.githubRepo ? `<li><strong>GitHub Repository</strong>: <a href="${escapeHtml(project.githubRepo)}" target="_blank">${escapeHtml(project.githubRepo)}</a></li>` : ""}
    ${!project.hasCustomContainer && project.challenges && project.challenges !== "None" ? `<li><strong>Challenges</strong>: ${escapeHtml(project.challenges)}</li>` : ""}
    ${!project.hasCustomContainer && project.solution && project.solution !== "None" ? `<li><strong>Solution</strong>: ${escapeHtml(project.solution)}</li>` : ""}
    ${project.socialLinks ? `
      <li><strong>Social Links</strong>: 
        ${project.socialLinks.facebook ? `<a target="_blank" href="${escapeHtml(project.socialLinks.facebook)}"><i class="bx bxl-facebook-circle"></i></a>` : ""}
      </li>
    ` : ""}
    ${project.editingDetails ? `
      <li><strong>Editing Details</strong>: <br>
        ${project.editingDetails.colorCorrection ? `<strong>Color Correction</strong>: ${escapeHtml(project.editingDetails.colorCorrection)}<br>` : ""}
        ${project.editingDetails.retouching ? `<strong>Retouching</strong>: ${escapeHtml(project.editingDetails.retouching)}<br>` : ""}
        ${project.editingDetails.effects ? `<strong>Effects</strong>: ${escapeHtml(project.editingDetails.effects)}<br>` : ""}
      </li>
    ` : ""}
  `;
  ...
  if (project.modelControls) {
    ...
    if (controlsPanel && controlsList) {
      ...
      Object.entries(project.modelControls).forEach(([key, value]) => {
        ...
        controlsHTML += `<li><strong>${escapeHtml(formattedKey)}</strong>: ${escapeHtml(value)}</li>`;
      });
